...
#             logger.error("Voyage AI library not available. Install with: pip install voyageai")
#             raise ImportError("voyageai library required")
...
#                 self.embedding_dim = len(test_result.embeddings[0])
#                 self.voyage_initialized = True
#                 logger.info(f"Voyage AI initialized successfully with model {self.model_name}")
#                 logger.info(f"Embedding dimension: {self.embedding_dim}")
#             else:
#                 raise Exception("Failed to get test embedding")
                
#         except Exception as e:
#             logger.error(f"Error initializing Voyage AI: {str(e)}")
#             raise
...
#         try:
#             if not self.voyage_initialized:
#                 raise Exception("Voyage AI not initialized")
            
#             if not texts:
#                 return np.array([]).reshape(0, self.embedding_dim)
            
#             # Voyage AI has rate limits, so we might need to batch
#             batch_size = 128  # Voyage AI's batch limit
#             all_embeddings = []
...
#                 else:
#                     raise Exception(f"Failed to get embeddings for batch {i//batch_size + 1}")
...
#     def add_document_embeddings(self, doc_id: str, pages: List[Dict]) -> Dict[str, int]:
#         try:
#             logger.info(f"Adding embeddings for document {doc_id} using Voyage AI")
...
#                 "embedding_dimension": self.embedding_dim,
#                 "model_name": self.model_name,
#                 "voyage_initialized": self.voyage_initialized,
#                 "unique_documents": len(set(
#                     metadata.get("doc_id", "") 
#                     for metadata in self.chunk_metadata.values()
#                 )),
...
# try:
#     embedding_service = EmbeddingService()
#     logger.info("Embedding service created successfully with Voyage AI and Claude Haiku")
# except Exception as e:
#     logger.error(f"Failed to create embedding service: {str(e)}")
#     embedding_service = None
